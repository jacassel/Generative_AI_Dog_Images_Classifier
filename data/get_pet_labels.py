...
def get_pet_labels(image_dir):
    ...
    """
    ## Retrieves the filenames from the folder pet_images/
   ...
   # Processes each of the files to create a dictionary where the key
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           raw_label = in_files[idx].lower()
           split_label = raw_label.split('_')
           pet_label = ' '.join(split_label[:-1])
           pet_label = pet_label.strip()
           
          # raw_label = in_files[idx].lower()
          # split_label = raw_label.split('_')
          # for item in range(0, len(split_label), 1):
          #  if split_label[item][0] == 0:
          #      split_label.pop(split_label[item])
